...
volatile uint8 *Tris_Register [(uint8)5] = {&TRISA,&TRISB,&TRISC,&TRISD,&TRISE};
volatile uint8 *Lat_Register [(uint8)5] = {&LATA,&LATB,&LATC,&LATD,&LATE};
volatile uint8 *Port_Register [(uint8)5] = {&PORTA,&PORTB,&PORTC,&PORTD,&PORTE};
...
Std_ReturnType gpio_pin_directoin_initialize (const Pin_Config_t *pin)
{
    ...
    {
        case GPIO_OUTPUT:
            (*Tris_Register[pin->port] &= ~((uint8)1<<pin->pin));
            ret = (Std_ReturnType)0X01;
            break;
...
Std_ReturnType gpio_pin_read_direction (const Pin_Config_t *pin, Direction_t *dir)
{
    ...
    {
        *dir = ((*Tris_Register[pin->port]>>pin->pin) & 1);
        ret = (Std_ReturnType)0X01;
    }
    return ret;
}
Std_ReturnType gpio_pin_write_logic (const Pin_Config_t *pin, Logic_t logic)
{
    ...
    {
        case GPIO_LOW:
            (*Lat_Register[pin->port] &= ~((uint8)1<<pin->pin));
            ret = (Std_ReturnType)0X01;
            break;
        case GPIO_HIGH:
            (*Lat_Register[pin->port] |= ((uint8)1<<pin->pin));
...
Std_ReturnType gpio_pin_read_logic (const Pin_Config_t *pin, Logic_t *logic)
{
    ...
    {
        * logic = ((*Port_Register[pin->port]>>pin->pin) & 1);
        ret = (Std_ReturnType)0X01;
    }
    return ret;
}
